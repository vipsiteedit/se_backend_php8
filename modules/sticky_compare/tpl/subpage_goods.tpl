...
<?php endforeach; ?>
   ...
   <div class="sticky_compare-compare_items__header"> 
    <?php foreach($section->product as $pr): ?>
     <div class="sticky_compare-item_header" data-item-id="<?php echo $pr->id ?>" data-shop-item="">
      <div class="sticky_compare-item_description">
       ...
       <div class="sticky_compare-item_title">
        <a class="sticky_compare-item_title_text" href="<?php echo $pr->link ?>" title="<?php echo $pr->name ?>" data-shop-item-title=""><?php echo $pr->name ?></a> 
        <span data-shop-item-article="" style="display: none;"><?php echo $pr->article ?></span>
       </div>
      </div>
      <?php if($pr->price>0): ?>
      ...
      <?php else: ?>
        <button type="button" class="buttonSend addcart goods_detail-send_question__button btn btn-default">Заказать</button>
        <a class="sticky_compare-item__button_remove" title="<?php echo $section->language->lang023 ?>" data-remove-id="<?php echo $pr->id ?>" href="?del_compare=<?php echo $pr->id ?>">
        <?php echo $section->language->lang024 ?></a>
      <?php endif; ?>
     </div>
...
<?php else: ?>
  ...
  <a class="sticky_compare-empty_list__goto_catalog" href="<?php echo $__data->getPagePattern($section->parametrs->param3) ?>"><?php echo $section->language->lang019 ?></a>
 </div>
