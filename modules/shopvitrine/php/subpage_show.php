...
if (!empty($selected) && !empty($price_fields['modifications']) {
	$plugin_modifications = new plugin_shopmodifications($viewgoods);
	$descr = $plugin_modifications->getDescription($selected);
	if (!empty($descr['description']))
		$price_fields['text'] = $descr['description'];
	if (!empty($descr['article']))
		$price_fields['article'] = $descr['article'];
}
...
if (!empty($price_fields['brand'])) {
    ...
    $brand_link = seMultiDir() . '/' . $_page . '/brand/' . urlencode($price_fields['brand_code']) . SE_END;
...
$product_link = $price_fields['link']; 
