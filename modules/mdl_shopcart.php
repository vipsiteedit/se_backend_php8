<?php
function module_shopcart($razdel, $section = null)
{
 ...
 if (strpos(dirname(__FILE__),'/lib/modules'))
   $__MDL_URL = 'lib/modules/shopcart';
 else $__MDL_URL = 'modules/shopcart';
 $__MDL_ROOT = dirname(__FILE__).'/shopcart';
 ...
 $total_weight_goods = $total['showWeight'];
 $total_volume_goods = $total['showVolume'];
 ...
 if ($ajax){ 
     ...
     if (isRequest('countitem') || isRequest('delcartname') || isRequest('cart_clear') || isRequest('getcart')){
         ...
         foreach ($incart as $val){
             if (!floatval($val['price'])) $is_price_null = true;
             $product = array();
             $product['id'] = $val['key'];
             $product['count'] = $val['count'];
             $product['sum'] = (floatval($val['price'])) ? $val['newsum'] : strval($section->language->lang094);
             ...
             $product['oldprice'] = (floatval($val['price'])) ? $val['oldprice'] : strval($section->language->lang094);
             $product['newprice'] = (floatval($val['price'])) ? $val['newprice'] : strval($section->language->lang094);
     ...
     else $ajax_response['total'] = strval($section->language->lang094);    
