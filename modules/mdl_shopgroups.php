<?php
function module_shopgroups($razdel, $section = null)
{
 ...
 if (strpos(dirname(__FILE__),'/lib/modules'))
   $__MDL_URL = 'lib/modules/shopgroups';
 else $__MDL_URL = 'modules/shopgroups';
 $__MDL_ROOT = dirname(__FILE__).'/shopgroups';
 ...
 $shopcatgr = $psg->checkUrl(trim($section->parametrs->param27));
 ...
 //BeginSubPagepagebrand
 $__module_subpage['pagebrand']['admin'] = "";
 $__module_subpage['pagebrand']['group'] = 0;
 $__module_subpage['pagebrand']['form'] =  '';
 if($razdel == $__data->req->razdel && !empty($__data->req->sub)
 && $__data->req->sub=='pagebrand' && file_exists($__MDL_ROOT . "/tpl/subpage_pagebrand.tpl")){
	include $__MDL_ROOT . "/php/subpage_pagebrand.php";
	ob_start();
	include $__data->include_tpl($section, "subpage_pagebrand");
	$__module_subpage['pagebrand']['form'] =  ob_get_contents();
	ob_end_clean();
 } //EndSubPagepagebrand
 //Final PHP
