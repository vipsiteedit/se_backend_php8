...
class plugin_shopgoods
{
	...
	public function getGoodsFeatures($id_goods)
	{
		...
		if (!empty($id_goods)) {
			...
			if ($show_feature) {
				...
				$sf->groupBy('sf.id, sf.name, sf.type, sf.measure, sf.image, sf.description, sfg.id, sfg.name, sfg.image, sfg.description, sfg.sort, sf.sort');
	...
	public function getGoods($options = array(), $goods_name = '', $on_count = false)
	{
		...
		if (!$on_count) {
			...
			$price->groupBy('sp.id, sp.code, sp.article, sp.name, sp.note, sp.text, sp.presence_count, sp.presence, sp.step_count, sp.measure, sp.flag_hit, sp.flag_new, sp.price, sp.price_opt_corp, sp.price_opt, sp.discount, sp.max_discount, sp.curr, sp.title, sp.keywords, sp.description, sp.delivery_time, sp.signal_dt, sp.id_brand, sp.sort, sgs.code_gr, sg.name, sg.code_gr, spg.id_group');
		if ($on_count) {
	...
	public function showGoodsDescription($goods_id = '')
	{
		...
		$goods['title'] = (trim((string)$goods['title'])) ? htmlspecialchars($goods['title']) : $name;
		$goods['keywords'] = (trim((string)$goods['keywords'])) ? htmlspecialchars($goods['keywords']) : $name;
		$goods['description'] = (trim((string)$goods['description'])) ? htmlspecialchars($goods['description']) : htmlspecialchars($goods['note']);
	...
	public function getProjectList($id_product)
	{
		...
		$table_check = se_db_query("SHOW TABLES LIKE 'shop_price_news'");
	...
	public function viewgalleryImages($price_id, $size_prev, $size_mid, $size_full, $res = 's', $watermark = '', $pos = 'center', $level = 75)
	{
		...
		public function getGoodsImage($imagename = '', $size = 0, $res = 's', $watermark = '', $pos = 'center', $level = 75)
		{
			...
			$nofoto = (basename((string)$imagename) === 'no_foto.gif');
