<?php
	...
	class plugin_qtsms {
		...
		private function get_post_request($invars = array()) {
			...
			$invars['CLIENTADR']            = $_SERVER['REMOTE_ADDR'] ?? false;
			$invars['HTTP_ACCEPT_LANGUAGE'] = $_SERVER['HTTP_ACCEPT_LANGUAGE'] ?? false;
		...
		private function get_multipost_request($invars,$file_data) {
			...
			$filesize = (isset($file_data["name"]) && $file_data["name"] != "" && !empty($file_data["path"]) && file_exists($file_data["path"])) ? filesize($file_data["path"]) : 0;
			...
			if (isset($info['transfer-encoding']) && $info['transfer-encoding'] == 'chunked') {
				...
				do {
					...
					if (isset($info['content-encoding'])) {
						$chunk = gzinflate(substr($tmp,($pos+$add+10),$len));
						$str .= ($chunk === false) ? '' : $chunk;
					} else {
				...
				$inflated = gzinflate(substr($tmp,10));
				$str = ($inflated === false) ? '' : $inflated;
			}
			return $str;
		}
