...
class plugin_macros {
    ...
    public function __construct($user_id = 0, $order_id = 0, $payment_id = 0){
        ...
        if ($this->order_id) {
            ...
            $this->ORDER = se_db_fetch_assoc($query) ?: array();
            if (!empty($this->ORDER)) {
    ...
    public function summtostring($summa){
        ...
        $num2 = $nums[2] ?? '';
        $num1 = $nums[1] ?? '';
        if ($num7 != '') $rez .= $d['mill'][$num7]. ' ';
        if ($num6 != '') $rez .= $d['sot'][$num6]. ' ';
        if ($num5 != '' && $num5 != 1) $rez .= $d['dec'][$num5].' ';
        if ($num5 != '' && $num5 == 1) $rez .= $d['des'][$num4] . ' ' . $d['thou'][0]." ";
        if ($num4 != '' && $num5 != 1) $rez .= $d['mel'][$num4].' '.$d['thou'][$num4]." ";


        if ($num3 != '') $rez .= $d['sot'][$num3]." ";
        if ($num2 != '' && $num2 != 1) $rez .= $d['dec'][$num2]." ";
        if ($num2 != '' &&  $num2 == 1) $rez .= $d['des'][$num1]." ";
        if ($num1 != '' &&  $num2 != 1) $rez .= $d['edin'][$num1]." ";
    ...
    private function parseOrder($text){
        if ($this->order_id) {
            ...
            $nds = isset($this->main['nds']) ? (float)$this->main['nds'] : 0.0;
            $array_change = array('ORDER_DISCOUNT' => se_formatMoney($discount, $this->curr),
                ...
                'ORDER_SUMM_NOTAX'=>se_formatMoney($summ - $nds/(100 + $nds) * $summ, $this->curr),
                'ORDER.SUMM_NOTAX'=>str_replace(',','.',($summ - $nds/(100 + $nds) * $summ)),
                'ORDER_SUMM_WITH_TAX'=>se_formatMoney($summ + $nds/100 * $summ, $this->curr),
                'ORDER.SUMM_WITH_TAX'=>str_replace(',','.',($summ + $nds/100 * $summ)),
                'ORDER_SUMM_TAX_EXT'=>se_formatMoney($nds/100 * $summ, $this->curr),
                ...
                'ORDER_SUMMNDS'=>se_formatMoney($nds/(100 + $nds)*$summ, $this->curr),
                'ORDER_SUMM_TAX'=>se_formatMoney($nds/(100 + $nds) * $summ, $this->curr),
                'ORDER_TAX'=>round($nds),
            ...
            if (strpos($text, '[SHOP_ORDER_VALUE_LIST]') !== false){
                ...
                $value_list .= '<td>[SHOPLIST.COUNT]</td><td>SUM([SHOPLIST.SUMMA]-[SHOPLIST.DISCOUNT])</td>';
