<?php
include 'connector/php/config.php';
...
class TinyImageManager {
	...
	function TinyImageManager() {
		...
		if (!file_exists($this->dir['files'])) mkdir($this->dir['files']);
		...
		switch ($_POST['action']) {
			...
			//Создать папку
			case 'changewidth':
					$_SESSION['editor_image_upload_width'] = intval($_POST['maxwidth']);
					break;
			case 'newfolder':
			...
			case 'showdir':
				...
				echo $this->ShowDir($path, $_POST['pathtype']);  // pathtype - "images"
				exit();
			break;
			
			//Загрузить изображение
			case 'uploadfile':
				if (!$_POST['maxwidth']) $maxwidth = $_SESSION['editor_image_upload_width'];
				else $maxwidth = intval($_POST['maxwidth']);
			    //$fp = fopen("test.log", "w+");
				//fwrite($fp, $maxwidth);
				//fclose($fp);
				echo $this->UploadFile($_POST['path'], $_POST['pathtype'], $maxwidth);
				exit();
			break;
	...
	function UploadFile($dir, $type, $maxwidth = 0) {
		$dir = $this->AccessDir($dir, $type);
		if(!$dir) return false;
		...
		if(isset($_POST['Filename'])) {
			...
			// Если размер картинки больше максимальной $maxwidth,  то режем
			if ($maxwidth > 0 && $files[$file]['imageinfo'][0] > $maxwidth){
					$t = new Image_Toolbox($_FILES['Filedata']['tmp_name']);
					$t->newOutputSize($maxwidth, 0, 0, false, '#FFFFFF');
					$t->save($dir.'/'.$file, $extension, 80);
					$files[$file]['imageinfo'] = getimagesize($dir.'/'.$file);
			}
			else
		...
		//Файлы из обычной загрузки 
			...
			foreach ($ufiles['name'] as $k=>$v) {
				...
				// Если размер картинки больше максимальной $width,  то режем
				if ($maxwidth > 0 && $files[$file]['imageinfo'][0] > $maxwidth){
					$t = new Image_Toolbox($ufiles['tmp_name'][$k]);
					$t->newOutputSize($maxwidth, 0, 0, false, '#FFFFFF');
					$t->save($dir.'/'.$file, $extension, 80);
					$files[$file]['imageinfo'] = getimagesize($dir.'/'.$file);
				}
	...
	function ShowDir($dir, $type) {
		...
		foreach ($dir as $v) {
			...
			if (strpos($v['general']['filename'],'_prev.')) continue;
			if (!file_exists(DIR_ROOT.$v['general']['path'].'/'.$v['general']['filename'])){
				unlink(DIR_ROOT.$v['general']['path'].'/.thumbs/'.$v['general']['md5'].'_100_100_2.jpg');
				unlink(DIR_ROOT.$v['general']['path'].'/.thumbs/'.$v['general']['md5'].'_0_500_0.jpg');
				continue;
			}

				$thumb = $this->GetThumb($v['general']['path'],$v['general']['md5'],$v['general']['filename'],2,100,100);
				//print_r($v['general']['width']);
